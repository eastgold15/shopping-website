...
async function seedDatabase() {
  try {
    ...
    // 1. 插入图片数据
    console.log("插入图片数据...");
    await db.insert(imagesTable).values(imageData);
    ...
    // 更新子分类的parentId
    const electronicsCategory = insertedCategories.find(
    ...
    if (electronicsCategory) {
      await db
        ...
        .where(
          or(
            eq(categoriesTable.slug, "laptops"),
            eq(categoriesTable.slug, "smartphones"),
            eq(categoriesTable.slug, "audio"),
          ),
        );
    }
    ...
    const insertedImages = await db
      .select()
      .from(imagesTable)
      .where(eq(imagesTable.category, "product"));

    const productImageRelations = [];
    // MacBook Pro 关联笔记本图片
    const macbookProduct = insertedProducts.find(
      (p) => p.slug === "macbook-pro-14",
    );
    const laptopImage = insertedImages.find(
      (img) => img.fileName === "laptop-1.jpg",
    );
    ...
    const iphoneProduct = insertedProducts.find(
      (p) => p.slug === "iphone-15-pro",
    );
    const phoneImage = insertedImages.find(
      (img) => img.fileName === "phone-1.jpg",
    );
    ...
    // AirPods Pro 关联耳机图片
    const airpodsProduct = insertedProducts.find(
      (p) => p.slug === "airpods-pro-3rd",
    );
    const headphonesImage = insertedImages.find(
      (img) => img.fileName === "headphones-1.jpg",
    );
    ...
    if (productImageRelations.length > 0) {
      await db.insert(productImagesTable).values(productImageRelations);
    }

    // 6. 插入广告数据
    console.log("插入广告数据...");
    const bannerImage =
      insertedImages.find((img) => img.category === "advertisement") ||
      imageData.find((img) => img.category === "advertisement");
    advertisementData[0].image = bannerImage?.url || "";
    await db.insert(advertisementsTable).values(advertisementData);

    // 7. 插入合作伙伴数据
    console.log("插入合作伙伴数据...");
    const partnerImage =
      insertedImages.find((img) => img.category === "partner") ||
      imageData.find((img) => img.category === "partner");
    partnerData[0].image = partnerImage?.url || "";
    await db.insert(partnersTable).values(partnerData);

    // // 7. 插入网站配置数据
    // console.log('插入网站配置数据...');
    // await db.insert(siteConfigTable).values(siteConfigData);
