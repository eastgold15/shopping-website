...
export const insertSiteConfigSchema = createInsertSchema(siteConfigTable, {
  key: z.string().min(1, "配置键名不能为空").max(100, "配置键名不能超过100个字符"),
  category: z.string().min(1, "配置分类不能为空"),
});
export const updateSiteConfigSchema = createUpdateSchema(siteConfigTable, {
  key: z.string().min(1, "配置键名不能为空").max(100, "配置键名不能超过100个字符").optional(),
...
export type InsertSiteConfigDto = z.infer<typeof siteConfigModel.insertSiteConfigDto>;  // 请求用
export type UpdateSiteConfigDto = z.infer<typeof siteConfigModel.updateSiteConfigDto>;  // 请求用
export type BatchUpdateSiteConfigDto = z.infer<typeof siteConfigModel.batchUpdateSiteConfigDto>;  // 批量更新用
