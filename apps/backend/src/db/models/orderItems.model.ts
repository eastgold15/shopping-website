...
export const updateOrderItemsSchema = createUpdateSchema(orderItemsTable, {
	...
	totalPrice: z
		.string()
		.regex(/^\d+(\.\d{1,2})?$/, "总价格式不正确")
		.optional(),
	finalPrice: z
		.string()
		.regex(/^\d+(\.\d{1,2})?$/, "最终价格格式不正确")
		.optional(),
	refundableQuantity: z
		.number()
		.int()
		.min(0, "可退款数量不能为负数")
		.optional(),
...
export const orderItemsModel = {
	...
	// 创建订单项请求参数
	insertOrderItemsDto: insertOrderItemsSchema.omit({
		id: true,
		createdAt: true,
		updatedAt: true,
	}),

	updateOrderItemsDto: updateOrderItemsSchema.omit({
		id: true,
		orderId: true,
		productId: true,
		createdAt: true,
...
export type InsertOrderItemsDto = z.infer<
	typeof orderItemsModel.insertOrderItemsDto
>; // 请求用
export type UpdateOrderItemsDto = z.infer<
	typeof orderItemsModel.updateOrderItemsDto
>; // 请求用
export type SelectOrderItemsType = z.infer<
	typeof orderItemsModel.selectOrderItemsTable
>; // 查询返回原始类型
export type OrderItemsListQueryDto = z.infer<
	typeof orderItemsModel.queryOrderItemsListDto
>;
export type OrderItemsByOrderIdQueryDto = z.infer<
	typeof orderItemsModel.queryOrderItemsByOrderIdDto
>;
export type BatchInsertOrderItemsDto = z.infer<
	typeof orderItemsModel.batchInsertOrderItemsDto
>;
export type UpdateRefundQuantityDto = z.infer<
	typeof orderItemsModel.updateRefundQuantityDto
>;
