...
export const productsSchema = pgTable("products", {
  ...
  comparePrice: decimal("compare_price", { precision: 10, scale: 2 }).default("0.00"), // 商品原价/对比价格
  ...
  categoryId: integer("category_id").references(() => categoriesSchema.id).default(-1), // 所属分类ID
...
export const ordersSchema = pgTable("orders", {
  ...
  shippingCost: decimal("shipping_cost", { precision: 10, scale: 2 }).default("0.00"), // 运费
...
export const orderItemsSchema = pgTable("order_items", {
  ...
  orderId: integer("order_id").references(() => ordersSchema.id).notNull(), // 关联订单ID
  productId: integer("product_id").references(() => productsSchema.id).notNull(), // 关联商品ID
