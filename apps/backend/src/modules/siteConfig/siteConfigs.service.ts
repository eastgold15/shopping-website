...
import {
	type BatchUpdateSiteConfigDto,
	type InsertSiteConfigDto,
	type SelectSiteConfigType,
	type SiteConfigListQueryDto,
	siteConfigTable,
	type UpdateSiteConfigDto,
} from "../../db/models/siteConfig.model";
...
> {
	 ...
	 */
	async getList(
		params: SiteConfigListQueryDto,
	): Promise<PageData<SelectSiteConfigType>> {
		const {
			page = 1,
			pageSize = 10,
			sortBy = "createdAt",
		...
		return await this.findPaginated({ page, pageSize }, queryOptions);
	}
