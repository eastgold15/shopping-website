...
export const siteConfigsController = new Elysia({ prefix: '/site-configs' })
  ...
  // 根据键获取配置
  .get('/:key', async ({ params: { key }, siteConfigsService }) => {
    try {
  ...
  // 创建配置
  .post('/', async ({ body, siteConfigsService }) => {
    try {
  ...
  // 更新配置
  .put('/:key', async ({ params: { key }, body, siteConfigsService }) => {
    try {
  ...
  // 删除配置
  .delete('/:key', async ({ params: { key }, siteConfigsService }) => {
    try {
  ...
  // 批量更新配置
  .patch('/batch', async ({ body, siteConfigsService }) => {
    try {
      const updatedConfigs = await siteConfigsService.batchUpdateConfigs(body);
  ...
  // 初始化默认配置
  .post('/initialize', async ({ siteConfigsService }) => {
    try {
      const initializedConfigs = await siteConfigsService.initializeDefaultConfigs();
