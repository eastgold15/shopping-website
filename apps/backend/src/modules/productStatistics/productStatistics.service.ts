...
import { PopularProductItem, ProductOverviewStats } from "../../db/models/productStatistics.model";
...
export class ProductStatisticsService extends BaseService<any, any, any> {
   ...
   * 获取热门商品统计
   */
  static async getPopularProducts(query: { limit?: number } = {}): Promise<PopularProductItem[]> {
    try {
      ...
      popularProducts.forEach((product) => {
        product.totalScore = product.viewCount + (product.searchCount * 1.5) + (product.favoriteCount * 2);
      });
