 ...
 * 处理商品统计相关的HTTP请求
 */
export const productStatisticsController = new Elysia({ prefix: "/product-statistics", tags: ["商品统计"] })
  .onBeforeHandle(() => {
    // 可以在这里添加全局的前置处理逻辑
  })
  ...
  .get(
    ...
    },
    {
      detail: {
        summary: "获取商品概览统计",
        description: "获取商品总数、上架商品数、推荐商品数、低库存商品数等概览数据",
  ...
  .get(
    "/recent",
    async ({ query }) => {
      const result = await ProductStatisticsService.getRecentProducts(query.limit);
      return commonRes(result);
    },
    {
      query: t.Object({
