import { and, asc, count, desc, eq, ilike, or, sql, getTableColumns } from 'drizzle-orm';
import { db } from '../../db/connection';
import { ordersSchema, orderItemsSchema, refundsSchema, productsSchema } from '../../db/schema/schema';
...
export class OrdersService {
   ...
   * 更新订单物流信息
   */
  async updateOrderShipping(id: number, trackingNumber: string, shippingMethod?: string) {
    try {
      const updateData: any = {
        trackingNumber,
        updatedAt: new Date()
   ...
   * 创建退款申请
   */
  async createRefund(orderId: number, amount: string, reason: string, refundMethod?: string) {
    try {
      // 检查订单是否存在
      const order = await db
        .select()
