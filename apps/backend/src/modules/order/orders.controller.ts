 ...
 * 处理订单相关的HTTP请求
 */
export const ordersController = new Elysia({ prefix: '/orders', tags: ['订单'] })
  ...
  // 更新订单状态
  .patch('/:id/status', async ({ params: { id }, body, ordersService }) => {
    const order = await ordersService.updateOrderStatus(id, body.status, body.notes);
    return {
      success: true,
      data: order,
      message: '订单状态更新成功'
  ...
  // 更新订单物流信息
  .patch('/:id/shipping', async ({ params: { id }, body, ordersService }) => {
    const order = await ordersService.updateOrderShipping(id, body.trackingNumber, body.shippingMethod);
    return {
      success: true,
      data: order,
      message: '物流信息更新成功'
  ...
  .post('/:id/refunds', async ({ params: { id }, body, ordersService }) => {
    const refund = await ordersService.createRefund(id, body.amount, body.reason, body.refundMethod);
    return {
      success: true,
      data: refund,
      message: '退款申请创建成功'
  ...
  // 处理退款申请
  .patch('/refunds/:refundId', async ({ params: { refundId }, body, ordersService }) => {
    const refund = await ordersService.processRefund(refundId, body.status, body.notes);
    return {
      success: true,
      data: refund,
      message: '退款申请处理成功'
  ...
  .get('/statistics', async ({ query, ordersService }) => {
    const statistics = await ordersService.getOrderStatistics(query.startDate, query.endDate);
    return {
      success: true,
      data: statistics
    };
