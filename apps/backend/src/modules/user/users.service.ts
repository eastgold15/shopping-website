import { and, count, desc, asc, like, eq, inArray, sql } from 'drizzle-orm';
...
import { ServiceResponse, PaginatedServiceResponse } from '@backend/utils/services/types';

export class UserService extends BaseService<UserEntity, CreateUser, UpdateUser> {
  constructor() {
   ...
   * 根据用户名查找用户
   */
  async findByUsername(username: string): Promise<ServiceResponse<UserEntity | null>> {
    try {
   ...
   * 根据邮箱查找用户
   */
  async findByEmail(email: string): Promise<ServiceResponse<UserEntity | null>> {
    try {
      const result = await db
        .select()
        .from(userSchema)
   ...
   * 获取用户列表（带搜索和筛选）
   */
  async findUsers(filters: UserQueryFilters): Promise<ServiceResponse<UserEntity[]>> {
    try {
      const conditions = [];

      if (filters.search) {
        conditions.push(
          like(userSchema.username, `%${filters.search}%`)
        );
      }
