...
const getProductComparePrice = (product: any) => {
	...
	if (
		...
		return product.price.originalPrice > getProductPrice(product)
			? product.price.originalPrice
			: null;
	}
	return null;
	...
	try {
		...
		if (res.code === 200) {
			...
			product.value = {
				...
				description:
					"这是一款经典的棉质T恤，采用100%纯棉材质制作，舒适透气，适合日常穿着。",
				...
				fullDescription:
					"这款经典棉质T恤采用优质100%纯棉面料，经过精心设计和制作。面料柔软舒适，透气性好，适合各种场合穿着。简约的设计风格，经典的剪裁，让您在任何时候都能展现出优雅的气质。无论是日常休闲还是运动健身，都是您的理想选择。",
			} as any;
...
const fetchRelatedProducts = async () => {
	try {
		...
		if (res && res.code === 200) {
			// 过滤掉当前商品
			relatedProducts.value = res.data?.items
