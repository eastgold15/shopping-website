  ...
  <div class="p-6">
    ...
    <div class="flex justify-between items-center mb-6">
      ...
      <div class="flex gap-3">
        <Button label="保存草稿" icon="pi pi-save" severity="secondary" @click="triggerSaveDraft" :loading="saving" />
            ...
            <template #content>
              <div class="space-y-4">
                ...
                <FormField v-slot="$field" name="categoryId" class="flex flex-col gap-1">
                  ...
                  <TreeSelect v-model="$field.value" :options="treeData" placeholder="请选择商品分类" class="w-full"
            ...
            <template #content>
              <div class="space-y-4">
                ...
                <FormField v-slot="$field" name="images" class="flex flex-col gap-1">
                  ...
                  <div class="space-y-4">
                    <!-- 选择图片按钮 -->
                    <Button @click="showImageSelector = true" icon="pi pi-images" label="选择图片" outlined />
                    ...
                    <div v-if="$field.value.length > 0" class="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div v-for="(image, index) in $field.value" :key="index" class="relative group">
                        ...
                        <Button @click="removeImage(index)" icon="pi pi-times" size="small" severity="danger" rounded
            ...
            <template #content>
              <div class="space-y-4">
                ...
                <div class="flex flex-col gap-1">
                  ...
                  <div class="flex flex-wrap gap-2">
                    <div v-for="(color, index) in initialValues.colors" :key="index"
                      ...
                      <span>{{ color }}</span>
                      <Button icon="pi pi-times" size="small" text @click="initialValues.colors.splice(index, 1)" />
                    </div>
                    <div class="flex items-center gap-2">
                      <InputText v-model="newColor" placeholder="添加颜色" class="w-32" @keyup.enter="addColor"
                        @keydown.stop @keypress.stop />
                ...
                <div class="flex flex-col gap-1">
                  ...
                  <div class="flex flex-wrap gap-2">
                    <div v-for="(size, index) in initialValues.sizes" :key="index"
                      class="flex items-center gap-2 bg-gray-100 px-3 py-1 rounded-full">
                      <span>{{ size }}</span>
                      <Button icon="pi pi-times" size="small" text @click="initialValues.sizes.splice(index, 1)" />
                    </div>
                    <div class="flex items-center gap-2">
                      <InputText v-model="newSize" placeholder="添加尺寸" class="w-32" @keyup.enter="addSize" @keydown.stop
                        @keypress.stop />
                ...
                <div class="flex flex-col gap-1">
                  ...
                  <input type="hidden" name="materials" :value="JSON.stringify(initialValues.materials)" />
                  <div class="flex flex-wrap gap-2">
                    <div v-for="(material, index) in initialValues.materials" :key="index"
                      class="flex items-center gap-2 bg-gray-100 px-3 py-1 rounded-full">
                      <span>{{ material }}</span>
                      <Button icon="pi pi-times" size="small" text @click="initialValues.materials.splice(index, 1)" />
                    </div>
                    <div class="flex items-center gap-2">
                ...
                <div class="flex flex-col gap-1">
                  ...
                  <input type="hidden" name="features" :value="JSON.stringify(initialValues.features)" />
                  <div class="flex flex-wrap gap-2">
                    <div v-for="(feature, index) in initialValues.features" :key="index"
                      class="flex items-center gap-2 bg-gray-100 px-3 py-1 rounded-full">
                      <span>{{ feature }}</span>
                      <Button icon="pi pi-times" size="small" text @click="initialValues.features.splice(index, 1)" />
                    </div>
                    <div class="flex items-center gap-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <FormField v-slot="$field" name="dimensions" class="flex flex-col gap-1">
                    ...
                    <InputText v-model="$field.value" placeholder="例如: 30x20x10 cm" class="w-full" />
                    <Message v-if="$field?.invalid" severity="error" size="small" variant="simple">
                      {{ $field.error?.message }}</Message>
                  </FormField>
                </div>
                ...
                <FormField v-slot="$field" name="videos" class="flex flex-col gap-1">
                  ...
                  <div class="space-y-2">
                    <div v-for="(video, index) in $field.value" :key="index" class="flex items-center gap-2">
                      <InputText v-model="$field.value[index]" placeholder="请输入视频URL" class="flex-1" />
                      <Button icon="pi pi-times" size="small" severity="danger"
                        @click="$field.value.splice(index, 1)" />
                    </div>
                    <Button icon="pi pi-plus" label="添加视频" size="small" outlined @click="$field.value.push('')" />
                  </div>
                  <Message v-if="$field?.invalid" severity="error" size="small" variant="simple">{{
                    $field.error?.message }}</Message>
                </FormField>
        ...
        <div class="space-y-6">
          ...
          <Card>
            ...
            <template #content>
              <div class="space-y-4">
                <!-- 销售价格 -->
                <FormField v-slot="$field" name="price" :resolver="priceResolver" class="flex flex-col gap-1">
                  <label class="block text-sm font-medium text-gray-700 mb-2">销售价格 *</label>
                  <InputNumber v-model="$field.value" mode="currency" currency="USD" locale="en-US" class="w-full" />
                  <Message v-if="$field?.invalid" severity="error" size="small" variant="simple">{{
                    $field.error?.message }}</Message>
                </FormField>
          ...
          <Card>
            ...
            <template #content>
              <div class="space-y-4">
                <!-- 库存数量 -->
                <FormField v-slot="$field" name="stock" :resolver="stockResolver" class="flex flex-col gap-1">
